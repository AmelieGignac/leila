<?php
  // Tableau contenant les noms des langues
  $nomsLangues = [
    "fr"  => "Français",
    "en"  => "English",
    "ru"  => "русский язык",
  ...
  foreach($contenuDossierTextes as $nomDossier) {
    if($nomDossier != '.' && $nomDossier != '..') {
      //$languesDisponibles.push($nomDossier); // Pas bon en PHP
      //array_push($languesDisponibles, $nomDossier); // Bon
      $languesDisponibles[] = $nomDossier; // meilleur
    }
  }
